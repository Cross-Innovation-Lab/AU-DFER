...
class VideoClsDatasetFrame(Dataset):
    ...
    def __init__(self, anno_path, data_path, mode='train', clip_len=8,
        ...
        self.args = args
        self.all_AU_dict = {}
        if self.args.data_set=='DFEW':
            with open("./saved/data/dfew/AU_DFEW.txt", "r") as f:
                data = f.readlines()
            for sample in data:
                line = sample[:-1].split(' ')[1:]
                sample_name = line[0]
                for i in range(1,19):
                    line[i] = int(line[i])
                self.all_AU_dict[sample_name] = line[1:]
        elif self.args.data_set == 'FERV39k':
            with open("./saved/data/ferv39k/AU_FERV39K.txt", "r") as f:
                data = f.readlines()
            for sample in data:
                line = sample[:-1].split(' ')[1:]
                sample_name = line[0]
    ...
    def __getitem__(self, index):
        if self.mode == 'train':
            ...
            try:
                buffer, au_labels = self.load_video(sample, sample_rate_scale=scale_t)  # T H W C
            ...
            if len(buffer) == 0:
                while len(buffer) == 0:
                    ...
                    index = np.random.randint(self.__len__())
                    sample = self.dataset_samples[index]
                    buffer, au_labels = self.load_video(sample, sample_rate_scale=scale_t)
            ...
            if args.num_sample > 1:
                ...
                buffer = self._aug_frame(buffer, args)
                #print('labels returned',self.label_array[index])
                #print('au_labels',au_labels)
            return buffer, self.label_array[index], index, {}, torch.tensor(au_labels)

        elif self.mode == 'validation':
            sample = self.dataset_samples[index]
            buffer, au_labels = self.load_video(sample)
            if len(buffer) == 0:
                while len(buffer) == 0:
                    warnings.warn("video {} not correctly loaded during validation".format(sample))
                    index = np.random.randint(self.__len__())
                    ...
                    buffer, au_labels = self.load_video(sample)
            buffer = self.data_transform(buffer)
            return buffer, self.label_array[index], sample, torch.tensor(au_labels)
            ...
            buffer = self.data_transform(buffer)
            return buffer, self.test_label_array[index], sample, chunk_nb, split_nb, torch.tensor(au_labels)
    ...
    def load_video(self, sample, sample_rate_scale=1):
        ...
        if self.mode == 'test':
            ...
            if self.args.data_set=='DFEW':
                sample_id = fname.split('/')[-1]
                while frame_count<1025:
                    if sample_id+'/'+str(frame_count) in self.all_AU_dict:
                        AU_labels.append(self.all_AU_dict[sample_id+'/'+str(frame_count)])
                        frame_count+=1
                    else:
                        break
            elif self.args.data_set=='FERV39k':
                fname = fname.split('/')
                if int(fname[-1])<10:
                    sample_id = fname[-3]+'/'+fname[-2]+'/'+fname[-1] + '/' + '0' + str(frame_count)
                else:
                    sample_id = fname[-3]+'/'+fname[-2]+'/'+fname[-1] + '/' + str(frame_count)
                while frame_count<1025:
                    if sample_id in self.all_AU_dict:
                        AU_labels.append(self.all_AU_dict[sample_id])
                        frame_count+=1
                    else:
                        break
            '''
            ...
            for i in range(0,18):
                ...
                if count>=len(AU_labels)/2:
                    single_line.append(1.0)
                else:
                    single_line.append(0.0)
            return buffer,single_line
        ...
        if self.args.data_set=='DFEW':
            sample_id = fname.split('/')[-1]
            #print(sample_id+'/'+str(frame_count))
            #print(sample_id+'/'+str(frame_count) in self.all_AU_dict)
            while frame_count<1025:
                if sample_id+'/'+str(frame_count) in self.all_AU_dict:
                    AU_labels.append(self.all_AU_dict[sample_id+'/'+str(frame_count)])
                    frame_count+=1
...
class VideoReaderFrame:
    def __init__(self, video_dir, file_ext='jpg'):
        ...
        #print(sorted(glob.glob(os.path.join(video_dir, f'*.{file_ext}'))))
